# Практическое задание — Паттерны конкурентности
...
├── fanin_simple.go
├── fanin_advanced.go
├── workerpool_simple.go
└── workerpool_advanced.go
```

- Все файлы проходят `go run -race` без предупреждений
- Простые задачи: вывод совпадает с ожидаемым
- Сложные задачи: программа корректно завершается, нет зависших горутин
